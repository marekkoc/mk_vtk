...
if __name__ == "__main__":
    ...
    #  wyznaczam indeksy w obrazie rastrowym punktow bifurjacji odczytanych
    # z zredukowanego (po usunieciu grubych galezi) pliku ves
    ...
    appendedpolys.SetNumberOfInputs(2*Kseg+1)
    ...
    for k in range(Kseg):
        ...
        if 0: mkio.writeSTL( fn + '.stl', tube.GetOutput())
        if 0: mkio.writePolyData(fn + '.vtk', tube.GetOutput())

        appendedpolys.SetInputDataByNumber(2*k, tube.GetOutput())

        source = vtk.vtkSphereSource()
        source.SetCenter(*points[k, 3:])
        source.SetRadius(r_vox[k])
        source.Update()
        appendedpolys.SetInputDataByNumber(2*k+1, source.GetOutput())

    # Sfera dla root
    source = vtk.vtkSphereSource()
    source.SetCenter(*points[0, :3])
    source.SetRadius(r_vox[0])
    source.Update()
    appendedpolys.SetInputDataByNumber(2*Kseg, source.GetOutput())

    appendedpolys.Update()
    polydata = appendedpolys.GetOutput()

    fn = 'vtree_tra_all'
    if 0: mkio.writeSTL(fn + '.stl', polydata)
    if 0: mkio.writePolyData(fn + '.vtk', polydata)
